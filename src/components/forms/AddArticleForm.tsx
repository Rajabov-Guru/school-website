...
import { IArticle, IForm} from "../../types/mainTypes";
import {Stack, TextField} from "@mui/material";
import Typography from "@mui/material/Typography";
import Button from "@mui/material/Button";
import PhotoIcon from "@mui/icons-material/Photo";

interface IAddArticle extends  IForm{
    article?:IArticle;
    handleSubmit:(data:FormData, flag?:boolean)=>void;
}

const AddArticleForm:FC<IAddArticle> = ({handleAfter,article,handleSubmit}) => {
    ...
    function submit(){
        ...
        if(article && article.id) formData.append('id', article.id.toString());
        formData.append('title', title);
        formData.append('preview', preview);
        formData.append('text', text);
        formData.append('image', image);
        handleSubmit(formData,article?true:false);
        handleAfter();
    }
    ...
    return (
        <Stack spacing={2}>
            <Stack spacing={3}>
                <Typography variant={'h6'}>Добавить статью</Typography>
                <TextField
                    id="title"
                    label="Загловок"
                    variant="outlined"
